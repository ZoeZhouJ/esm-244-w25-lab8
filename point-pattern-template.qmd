...
plotting the distribution around obs points. sigma controls variance around the points, small sigma might overfit, bigger sigma is more generalized. 
...
The observed line is much higher than the bounds of the theoritical line. We have sufficient evidence to conclude the voles are not randomly distributed.  解：our obs is clustered! 
...
```{r}
class(gfunction_out)
gfunction_df <- as.data.frame(gfunction_out)
g_long <- gfunction_df %>% 
  pivot_longer(cols = c('obs','theo','lo','hi'), names_to = 'model', values_to = "G_r")

g_long %>% 
  ggplot(aes(x=r, y = G_r, group = model))+
...
lfunction_df <- as.data.frame(lfunction_out)
l_long <- lfunction_df %>% 
  pivot_longer(cols = c('obs','theo','lo','hi'), names_to = 'model', values_to = "L_r")
```


```{r}
l_long %>% 
  ggplot(aes(x=r, y = L_r, group = model))+
  geom_line(aes(color = model)) +
  theme_minimal() +
